...
var chilliController = { interval:30 , host:"1.0.0.1" , port:false , ident:'00' , ssl:false , uamService: true, redirect:false, typeUser: 'account_temp', accountServer: 'http://auth3.s-wifi.vn/getaccount', apType: 'Ruckus' };
...
chilliController.logon = function ()  {
...
//alert(resp.clientState);
//alert(resp.challenge);
//alert(typeof (resp.challenge));
	log('Entering logonStep 2');
	 if ( resp.clientState === chilliController.stateCodes.AUTH ) {
                log('logonStep2: Already connected. Aborting.');
                if (chilliController.redirect!=false){
                    window.location.href=chilliController.redirect;
                 }
	...
	if ( chilliController.uamService ) { /* MD5 CHAP will be calculated by uamService */
		...
		// Build command URL
		var url = chilliController.accountServer + '/?challenge=' + escape(challenge) + '&user-type=' + chilliController.typeUser + '&ap-type=' + chilliController.apType ;
...
chilliController.logonStep3 = function ( resp ) {
    ...
    var logonUrl = chilliController.urlRoot() + 'logon?username=' + resp.username + '&password=' + resp.password;
